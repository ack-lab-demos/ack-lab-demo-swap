...
async function getCurrentExchangeRate(): Promise<number> {
  try {
    const priceUpdates = await pythClient.getLatestPriceUpdates([
      SOL_USD_PRICE_ID,
    ]);
    ...
    const price =
      Number(solPriceData.price.price) * Math.pow(10, solPriceData.price.expo);

    logger.market("Fetched SOL/USD price from Pyth", {
      Price: `$${price.toFixed(2)}`,
      Confidence: `±$${(
        Number(solPriceData.price.conf) * Math.pow(10, solPriceData.price.expo)
      ).toFixed(2)}`,
    ...
    tools: {
      createSwapRequest: tool({
        ...
        inputSchema: z.object({
          ...
          recipientAddress: z
            .string()
            .describe("SOL address to receive the swapped SOL")
            .optional(),
        }),
        execute: async ({ usdcAmount }) => {
          logger.process("Creating swap request", {
          ...
          const { url: paymentRequestUrl, paymentRequestToken } =
            await swapServiceAgent.createPaymentRequest({
              id: paymentRequestId,
              amount: paymentUnits,
              description: `Swap ${usdcAmount} USDC for ${solAmount.toFixed(
                6
              )} SOL`,
            });
          ...
          logJwtIfEnabled(
            paymentRequestToken,
            "Decoded payment request token JWT payload"
          );
          ...
          return {
            ...
            description: `Swap ${usdcAmount} USDC for ~${solAmount.toFixed(
              6
            )} SOL at rate ${exchangeRate} USDC/SOL`,
      ...
      executeSwap: tool({
        ...
        inputSchema: z.object({
          receiptUrl: z
            .string()
            .url()
            .describe("The URL of the payment receipt JWT from the payment"),
          recipientAddress: z
            .string()
            .describe("SOL address to send the swapped SOL")
            .optional(),
        }),
        execute: async ({
          receiptUrl,
          recipientAddress = "7VQo3HWesNfBys5VXJF3NcE5JCBsRs25pAoBxD5MJYGp",
        }) => {
          ...
          const { receipt, paymentRequestId } =
            await swapServiceAgent.verifyPaymentReceipt(paymentReceipt);
          ...
          // Execute swap
          const swapResult = await executeSwapOnDex(
            pendingSwap.usdcAmount,
            pendingSwap.exchangeRate
          );
          ...
          const sendResult = await sendSolToAgent(
            recipientAddress,
            swapResult.solReceived
          );
...
// ==================== Swap User ====================
const callSwapService = swapUserAgent.createAgentCaller(
...
async function runSwapUser(message: string) {
  const result = await generateText({
    ...
    tools: {
      ...
      executePayment: tool({
        description:
          "Execute a USDC payment using a payment request URL received from the Swap Agent",
        inputSchema: z.object({
          paymentRequestUrl: z
            .string()
            .describe(
              "The URL of the payment request token received from the Swap Agent"
            ),
        }),
        execute: async ({ paymentRequestUrl }) => {
          ...
          try {
            const paymentRequestToken = await fetch(paymentRequestUrl).then(
              (res) => res.text()
            );

            logJwtIfEnabled(
              paymentRequestToken,
              "Payment request token JWT payload (before execution)"
            );

            const result = await swapUserAgent.executePayment(
              paymentRequestToken
            );
