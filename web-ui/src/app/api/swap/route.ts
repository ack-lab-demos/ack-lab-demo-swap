...
export async function POST(request: NextRequest) {
  try {
    ...
    // Decode payment token if JWT decoding is enabled
    if (DECODE_JWT && paymentTokenMatch?.[0]) {
      const decodedPayload = decodeJWT(paymentTokenMatch[0]);
      if (decodedPayload) {
        console.log('Decoded payment token JWT payload:', decodedPayload);
      }
    }
    
    // Also check for any other JWT patterns in the response
    if (DECODE_JWT) {
      const jwtPattern = /\b[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+\.[A-Za-z0-9-_]+\b/g;
      const jwtMatches = agentResponse.match(jwtPattern);
      if (jwtMatches) {
        jwtMatches.forEach((token, index) => {
          if (token !== paymentTokenMatch?.[0]) { // Don't decode payment token twice
            const decodedPayload = decodeJWT(token);
            if (decodedPayload) {
              console.log(`Decoded JWT token #${index + 1} from response:`, decodedPayload);
            }
          }
        });
      }
