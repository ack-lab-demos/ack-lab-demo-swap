...
export function SwapInterface() {
  ...
  const [priceSource, setPriceSource] = useState<'pyth' | 'fallback'>('pyth');
  const [, setLastPriceUpdate] = useState<Date | null>(null);
  ...
  useEffect(() => {
    const fetchPythPrice = async () => {
      try {
        ...
        setCurrentExchangeRate(data.price);
        setPriceConfidence(data.confidence);
        setPriceSource(data.source);
        setLastPriceUpdate(new Date(data.timestamp));
        console.log('[UI] Updated price from Pyth:', data);
      } catch {
