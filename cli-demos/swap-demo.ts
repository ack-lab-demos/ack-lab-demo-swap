...
async function executeSwapOnDex(usdcAmount: number, exchangeRate: number): Promise<{
  ...
  console.log(colors.cyan(`🔄 Executing swap on DEX...`))
  console.log(colors.gray(`   USDC: ${usdcAmount}`))
  console.log(colors.gray(`   Rate: ${exchangeRate} USDC/ETH`))
  console.log(colors.gray(`   ETH: ${ethAmount.toFixed(6)}`))
...
async function sendEthToAgent(recipientAddress: string, ethAmount: number): Promise<{
  ...
  console.log(colors.green(`💸 Sending ${ethAmount.toFixed(6)} ETH to ${recipientAddress}`))
    ...
    tools: {
      createSwapRequest: tool({
        ...
        execute: async ({ usdcAmount }) => {
          console.log(colors.magenta(">>> Creating swap request for:"), `${usdcAmount} USDC`)
          ...
          console.log(colors.yellow(">>> Payment token generated:"), paymentToken)
          console.log(colors.blue(">>> Exchange rate:"), `${exchangeRate} USDC/ETH`)
          console.log(colors.blue(">>> Expected ETH:"), `${ethAmount.toFixed(6)} ETH`)
          ...
          if (DECODE_JWT) {
            ...
            if (tokenParts.length === 3) {
              try {
                ...
                console.log(colors.cyan(">>> Decoded JWT payload:"), JSON.stringify(payload, null, 2))
              } catch {
                console.log(colors.gray(">>> Could not decode payment token as JWT"))
              }
            }
          }
    ...
    tools: {
      callSwapAgent: tool({
        ...
        execute: async ({ message }) => {
          console.log(colors.yellow(">>>> Calling swap agent:"), message)
          try {
            const response = await callAgent({ message })
            console.log(colors.blue(">>>> Swap agent response:"), response)
            ...
            if (paymentTokenMatch) {
              console.log(colors.green(">>>> Detected payment token:"), paymentTokenMatch[0])
            }
            
            return response
          } catch (error) {
            console.error(colors.red(">>>> Error calling swap agent:"), error)
            return {
              error: true,
      ...
      executePayment: tool({
        ...
        execute: async ({ paymentToken }) => {
          console.log(colors.magenta(">>>> Executing USDC payment for token:"), paymentToken)
          console.log(colors.gray("    Token length:"), paymentToken.length)
          console.log(colors.gray("    Token preview:"), paymentToken.substring(0, 20) + "...")
          ...
          try {
            ...
            console.log(colors.green(">>>> Payment successful! Receipt ID:"), result.receipt.id)
            ...
            console.log(colors.green("    Payment confirmed"))
            ...
            console.error(colors.red(">>>> Payment failed:"), error)
...
async function main() {
  ...
  console.log(colors.yellow("Exchange USDC for ETH using the Swap Agent"))
